...
const CreatePost = () => {
  ...
  const [postData, setPostData] = useState<FormInputs>({
    caption: "",
    postUrl: "",
  });
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (fileList && fileList.length > 0) {
      ...
      const imageFiles: File[] = filesArray.filter((file) =>
  ...
  const handleCreatePostClick = async () => {
    try {
      ...
      const response = await axiosInstance.post(
        `/createPost`,
        { ...postData, userId, postUrl: croppedImages, type },
        {
          headers: {
            ...
            <div className="bg-white rounded-lg p-4 shadow-md">
              <Cropper
                image={
                  ...
                  URL.createObjectURL(selectedFile[currentImageIndex])
                }
                crop={crop}
                zoom={zoom}
                aspect={2 / 2}
                onCropChange={(crop) =>
                  handleCropChange(crop, currentImageIndex)
                }
                onZoomChange={(zoom) =>
                  handleZoomChange(zoom, currentImageIndex)
                }
                ...
                <div className="flex justify-center space-x-4">
                  ...
                  <div className="flex justify-center space-x-4">
                    <button
                      ...
                      style={{
                        boxShadow:
                          "6px 6px 12px #c3c3c3, -6px -6px 12px #ffffff",
                      }}
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                  ...
                  {croppedImages.length > 1 && (
                    <>
                      ...
                      <button
                        onClick={() =>
                          setCurrentImageIndex((prevIndex) =>
                            prevIndex === croppedImages.length - 1
                              ? 0
                              : prevIndex + 1
                          )
                        }
