...
const Graph: React.FC = () => {
  ...
  useEffect(() => {
    axiosInstance.get('/admin/graphdata')
      .then(response => {
  ...
  const chartData = filter === 'monthly' ? {
    ...
    datasets: [
      {
        label: 'Users',
        data: monthlyUserCounts.length > 0 ? monthlyUserCounts : new Array(monthlyPostCounts.length).fill(0),
    ...
    datasets: [
      {
        label: 'Users',
        data: yearlyUserCounts.length > 0 ? yearlyUserCounts : new Array(yearlyPostCounts.length).fill(0),
  ...
  return (
    <div className="graph-container">
      <div className="filter-container">
        <select value={filter} onChange={(e) => setFilter(e.target.value as 'monthly' | 'yearly')} className="filter-select">
