...
function AddStory() {
 ...
 const [uploading, setUploading] = useState<boolean>(false);
  ...
  console.log("stories:", stories);
    ...
    const userStories = Array.isArray(stories) ? stories.filter((story) => story.userId._id === userId) : [];
    ...
    try {
     ...
     console.log(cloudinaryResponse,"res fromc lurd")
      ...
      if (cloudinaryResponse.status === 200 && imageUrl) {
        ...
        const response = await axiosInstance.post(`/createStory`, { storyImg: imageUrl, userId }, {
          headers: {
...
className="w-full h-full object-cover" />
      ...
      onClick={handleIConClick}
        ...
        style={{
          boxShadow: '3px 3px 6px rgba(0, 0, 0, 0.2), -3px -3px 6px rgba(255, 255, 255, 0.5)',
        }}
      >
        +
      </button>
