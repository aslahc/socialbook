...
function Notification() {
  ...
  const { getNotification, notifications, socket, setNotifications } =
    useSocketIO();
  ...
  const userDetails =
    notifications.length > 0 && notifications[notifications.length - 1].userData
      ? usersDetails.find(
          (user) =>
            user._id === notifications[notifications.length - 1].userData
        )
      : null;
          ...
          <div className="bg-white rounded-3xl my-1 mx-5 min-h-screen neumorphism p-6">
            ...
            {notifications
              ...
              .map((notification: any, index: number) => (
                <div
                  key={index}
                  className="bg-white rounded-lg shadow-lg p-4 flex items-center justify-between mb-4 neumorphism"
                >
                  <div className="flex items-center">
                    <div className="relative">
                      <img
                        src={
                          notification.sourceId.profileimg ||
                    ...
                    <div className="ml-4">
                      <p className="text-gray-800 font-bold">
                        {notification.sourceId.username ||
